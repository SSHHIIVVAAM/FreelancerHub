...
//     let apiUrl = `http://localhost:5022/api/Recruiter/GetAllMyProposals?pageIndex=1&pageSize=10&recruiterid=${recruiterId}`;

//     // Add filters to the API URL only if they have values
//     if (statusFilter) apiUrl += `&status=${statusFilter}`;
//     if (titleFilter) apiUrl += `&title=${titleFilter}`;
...
//         setIsLoading(false);
//       });
//   }, [recruiterId, statusFilter, titleFilter]); // Only depend on recruiterId for the initial API call
...
//   }, [recruiterId]); // Effect runs only once when component mounts
...
//         <h1 className="browse-title text-black text-start">Browse Jobs</h1>
...
//           {proposals.length === 0 ? (
//             <p className="text-white text-center">No job proposals available.</p>
//           ) : (
