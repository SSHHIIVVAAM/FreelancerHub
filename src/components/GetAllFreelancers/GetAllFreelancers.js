...
//                 </button>
//                 <button
//                   className=" custom-button btn btn-secondary"
//                   onClick={closeModal}
//                 >
//                   Close
//                 </button>
...
const GetAllFreelancers = () => {
  ...
  const fetchFreelancers = async (page) => {
    try {
      setLoading(true);
      const response = await fetch(
        `http://localhost:5022/api/Recruiter/GetAllFreelancers?pageIndex=${page}&pageSize=${pageSize}`,
        {
          method: "GET",
          headers: { Accept: "text/plain" },
        }
    ...
    <div className="background-image">
      <div className="container mt-5">
        ...
        {isModalOpen && (
          <div className="modal">
            <div className="modal-content">
              <h5>Send a message to Freelancer</h5>
              <textarea
                className="form-control"
                rows="4"
                value={message}
